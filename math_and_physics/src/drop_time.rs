...
fn compute_drop_time_logic(
    ...
    let correct_drop_time: f64 = (t_total) - ((2.0 * height) / GRAVITY).sqrt();
    ...
    while t <= t_total {
        ...
        let balloon_position: f64 = if t < input_drop_time {
            height
        } else {
            let t_since_drop: f64 = t - input_drop_time;
            height - (0.5 * GRAVITY * t_since_drop.powi(2))
        };
    ...
    DropTimeResult{
        hit: student_drop_time == rounded_correct_drop_time,
